...
addpath 'Z:\Pilot data\Exp1' %analysis script
...
for ii = 1:2
    for i = 2:height(cleanLogs)-40 %MEG version has 40 mini-blocks
...
if PHOTODIODE
    ...
    figure; plot(PhotodiodeSig); hold on; plot(PhotodiodeSigBin*photoDiodeThreshold)
    ...
    % jitters, not delays):
    PTBInaccuracy = intervalPhotodiode(1:length(intervalLogs))-(intervalLogs*1000); %removed the transposition in (intervalLogs*1000)
    figure()
