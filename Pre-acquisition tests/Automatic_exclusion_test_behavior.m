...
elseif ECoG
    if ((data.Hits)<(160*0.5)*0.70) & ((data.False_Alarm)>((1280*0.5)*0.3))
        %     warning('Hits and False Alarms limit has NOT been passed, need to exclude participant')
        questdlg(['Hits=',num2str(data.Hits),' and FA=',num2str(data.False_Alarm),', out of 80 targets and 640 non-targets/irrelevants, which means Hits and False Alarms limit has been passed, need to exclude participant'],'OK','OK','OK');
    elseif (data.Hits)<((160*0.5)*0.70)
        questdlg(['Hits=',num2str(data.Hits),', out of 80 targets, which means Hits are below 70%, need to exclude participant'],'OK','OK','OK');
        % warning('Hits are below 90%, need to exclude participant')
    elseif (data.False_Alarm)>((1280*0.5)*0.3)
        questdlg(['FA=',num2str(data.False_Alarm),', out of 640 non-targets/irrelevants, which means False Alarms are above 30%, need to exclude participant'],'OK','OK','OK');
        %     warning('False Alarms are above 10%, need to exclude participant')
    else
        questdlg('Participant`s behavioral scores are good','Message','OK','OK');
        %     disp('Participant`s behavioral scores are good')
...
if ECoG
if ((sum(TP_loc)) < (sum(localizer_all)*.65)) & ((sum(FP_loc)) > (sum(localizer_all)*.40))
%     warning('Hits and False Alarms limit has NOT been passed, need to exclude participant') 
     questdlg('Hits and False Alarms limit has been passed, need to exclude participant','OK','OK','OK');

elseif (sum(TP_loc)) < (sum(localizer_all)*.65)
     questdlg('Hits are below 65%, need to exclude participant','OK','OK','OK');
%     warning('Hits are below 85%, need to exclude participant')
elseif(sum(FP_loc)) > (sum(localizer_all)*.40)
     questdlg('False Alarms are above 40%, need to exclude participant','OK','OK');
