...
% For the ECoG team, the first jitter and fixation of each trial was
% removed. For the others, this needs to be taken into accout here:
if ~ECoG
    for ii = 1:2
        for i = 2:height(miniBlocks)-max(cleanLogs.miniBlock)
            if miniBlocks.miniBlock(i,1) > miniBlocks.miniBlock(i-1,1)
                miniBlocks(i,:) = [];
            end
        end
    end
    % miniBlocks the ones of the first block too:
    miniBlocks(1:2,:) = [];
end
