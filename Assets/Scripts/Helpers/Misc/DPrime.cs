...
namespace Helpers.Misc
{
    ...
    /// [CHECK]
    /// </summary>
    public static class DPrime_Helper
    {

        // private static Dictionary<double, double> Z_Values = new Dictionary<double, double>();
        // - 3.99, .... 3.99
        private static List<double> Z_VALUES = new List<double>()
    {
         .00003         ,.00003       ,.00004       ,.00004       ,.00004       ,.00004       ,.00004       ,.00004       ,.00005       ,.00005
         ...
         ,.05592        ,.05705       ,.05821       ,.05938       ,.06057       ,.06178       ,.06301       ,.06426       ,.06552       ,.06681
         ,.06811        ,.06944       ,.07078       ,.07215       ,.07353       ,.07493       ,.07636       ,.07780       ,.07927       ,.08076
        ...
        public static double DPrime_H_FAs(double H01, double FA01)
        {
            return DPrime(FA01, H01);
        }

        /// <summary>
        /// Returns Z(φ1) - Z(φ0)
        /// </summary>
        public static double DPrime(double φ0, double φ1)
        {
            return Z(φ1) - Z(φ0);
        }

        public static double Z(double φ)
        {
            // Find the Z-Value corresponding to Φ
            double f_val = Z_VALUES.Aggregate((x, y) => Math.Abs(x - φ) < Math.Abs(y - φ) ? x : y);
            int f_idx = Z_VALUES.FindIndex(x => x == f_val);
            double z = (f_idx - 399) / (double)100;
            return z;
            /*
            // Look up Z
            φ = Math.Min(3.99, Math.Max(-3.99, φ));

            double f_index = Math.Round(φ, 2);

            int i = (int)(f_index * 100 - 399);
        ...
        public static void TestZ()
        {
            ...
            {
                double error = Math.Abs(y[i] - Z(x[i]));
                if (error > maxError)
                    maxError = error;
            }

            // Console.WriteLine("Maximum error: {0}", maxError);
            Debug.Log("Maximum error: " + maxError);
        }
