...
VIEW_DISTANCE = 119; % Default viewing distance (if no viewDist argument sent with the function call)
SCREEN_SIZE_CM = [78 44]; % screen [width, height] in centimeters, change it to fit your setting
viewDistanceBottomTop = [140 140]; % IN MM!!! Distance between the participant head and the top and bottom of the screen. Only needed if HEAD_FIXED on.
...
HEAD_FIXED = 1; % Head fixed must be set to 0 if remote mode
TOBII_EYETRACKER = 0; 
calibration_area = [0.88 0.83];

%% MEEG parameters:
EEG_MACHINE_HEX = 'BFF8';
