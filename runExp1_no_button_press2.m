...
global TRIGAUD_CODE_START WhereToRestart TRG_EXP_START_MSG_AUD TRG_EXP_END_MSG_AUD RESTART_PRACTICE_MESSAGE YesKey TOBII_EYETRACKER
...
function [ miniBlocks ] = runMiniBlocks( miniBlocks,TriggerMatrix )
    ...
    global bitsi_buttonbox LAB_ID MEEG fMRI ECoG  LPT_OBJECT LPT_ADDRESS TRG_RESPONSE TRG_STIM_END TRG_JITTER_START TRG_MB_ADD
    ...
    global VERBOSE PROGRESS_MESSAGE TEXTURES_ANIMAL_REWARD InterruptFlag exp_Interrupt_counter
    global PHOTODIODE ABORT_KEY Block_ctr WhereToRestart RestartFlag TOBII_EYETRACKER tobii_eyetracker tobii tobii_TimeCell
...
try
    ...
    while miniBlockNum <= size(miniBlocks,1)
        ...
        if (miniBlockNum > 1) && ECoG
            prog_message = sprintf(strcat(PROGRESS_MESSAGE, '\n\n%d/20'), miniBlockNum-1);
            texturePtr_animal = TEXTURES_ANIMAL_REWARD(1,(miniBlockNum-1));
            progress_bar_patient(miniBlockNum,prog_message, texturePtr_animal);
            % KbWait(compKbDevice,3)
            ...
            if miniBlockNum < size(miniBlocks,1) && miniBlockNum > 1
                ...
                if EYE_TRACKER
                    if ~TOBII_EYETRACKER
                        ...
                        % At the end of each block we get the
...
%         end
        ...
        for tr = 0 : miniBlocks{miniBlockNum,MINI_BLOCK_SIZE_COL} - 1
            ...
            while elapsedTime < TRIAL_DURATION + miniBlocks{miniBlockNum, TRIAL1_JITTER_TIME_COL + tr} - (refRate*(2/3))
                ...
                if PHOTODIODE && hasInput == TRUE && (GetSecs - miniBlocks{miniBlockNum,TRIAL1_BUTTON_PRESS_COL + tr}) > ...
                        (DIOD_DURATION*refRate - refRate*(2/3)) && TriggerOFF.Photo.Response == FALSE
                    turnPhotoTrigger('off');
        ...
        if(key==RESTART_KEY)
            if (fMRI)
                ...
                saveBlockToExcel(OUTPUT_TABLE,miniBlockNum,tr);
                importEyetrackerEDF(Block_ctr+1,RestartFlag,exp_Interrupt_counter)
                initEyetracker(Block_ctr+1) % Then, initialize the tracker again, with new file name:
