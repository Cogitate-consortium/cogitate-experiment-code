...
VIEW_DISTANCE = 80; % Default viewing distance (if no viewDist argument sent with the function call)
SCREEN_SIZE_CM = [34.5 19.5]; % screen [width, height] in centimeters, change it to fit your setting
viewDistanceBottomTop = [80 80]; % Distance between the participant head and the top and bottom of the screen. Only needed if HEAD_FIXED on.

%% Eyetracker parameters:
EYE_TRACKER = 1; % Must be set to 1 if recording with Eyetracker
DISTANCE_SCREEN_TRACKER = 300; % Distance in mm between the eyetracker lense and the computer screen. Only needed for REMOTE MODE!
...
%% ECoG parameters:
BIT_DURATION = 0.02; % Duration of single bit in ECoG audio triggers
...
DIOD_SIZE = 250; % Size of the square where the photodiode is presented (in pixels)
...
WINDOW_RESOLUTION = [10 10 1200/1.5 800/1.5];

%% Matrix generation
MATRIX_GENERATION = 0;
PREEXISTING_MATRICES = 1;
