...
if ABORTED
    ...
    fileName = sprintf('%s%c%s%c%s%c%s',pwd,filesep,DATA_FOLDER,filesep,[LAB_ID,num2str(subjectNum)],filesep,[LAB_ID,num2str(subjectNum),EYETRACKER_FILE_NAMING,num2str(Block_ctr),'_ABORTED',excelFormat]);
    FileExist = 1;
    numAdd = 1;
    while FileExist % As long as the file already exists, we add a new number to mark it:
        status = isfile(fileName); % If the file exists
        if status
            numAdd = numAdd + 1; % Add 1 to the file name
            fileName = sprintf('%s%c%s%c%s%c%s',pwd,filesep,DATA_FOLDER,filesep,[LAB_ID,num2str(subjectNum)],filesep,[LAB_ID,num2str(subjectNum),EYETRACKER_FILE_NAMING,num2str(Block_ctr),'_ABORTED_',num2str(numAdd),excelFormat]); % Rename the file
        else
            FileExist = 0; % If the file doesn't exist, mark it as such, so that we exit the while loop
        end
    end
elseif RestartFlag
    fileName = sprintf('%s%c%s%c%s%c%s',pwd,filesep,DATA_FOLDER,filesep,[LAB_ID,num2str(subjectNum)],filesep,[LAB_ID,num2str(subjectNum),EYETRACKER_FILE_NAMING,num2str(Block_ctr),'_RESTARTED',excelFormat]);
else
    fileName = sprintf('%s%c%s%c%s%c%s',pwd,filesep,DATA_FOLDER,filesep,[LAB_ID,num2str(subjectNum)],filesep,[LAB_ID,num2str(subjectNum),EYETRACKER_FILE_NAMING,num2str(Block_ctr),excelFormat]);
end

writetable(gazeTable, fileName);

% And finally, turning off the recordings and clearing the gazeData:
